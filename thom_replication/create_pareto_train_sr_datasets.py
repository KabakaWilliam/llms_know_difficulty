...
def get_task(name):
    ...
    if name == 'E2H-AMC':
        from utils.verification_math import compute_score, extract_solution
        ds = load_dataset("furonghuang-lab/Easy2Hard-Bench", "E2H-AMC")
        # extract the solution from the chain of thought
        def add_extracted_solution(example):
        ...
        if "eval" in ds and "test" not in ds:
            # rename eval split to test
            ds["test"] = ds.pop("eval")
...
def main(
    ...
    # --------- tasks ----------
    # MODIFIED FOR SELECTIVE INFERENCE - RESTORE ORIGINAL AFTER USE
    TASKS = [
        # "E2H-AMC",  # Disabled - will be handled by get_task patching
        # "openai_gsm8k",
    ...
    for TASK in TASKS:
        ...
        # MODIFIED: Only process train split for full regeneration
        selected_indices = {}
        if 'train' in ds:
            split = 'train'
            n = len(ds[split])
            ...
            print(f"⚠️  Warning: No 'train' split found in dataset {TASK}")
            continue
        ...
        # --- collect results ---
        # Only create results dict for train split
        results = {'train': {}}
        ...
        for inp, out in zip(inputs, outputs_all):
            ...
            if idx not in results[split]:
                ...
                results[split][idx] = {
                    ...
                    "generated_solutions": [],
                    "rating": ds[split][idx].get("rating", np.nan),
...
if __name__ == "__main__":
    ...
    MODEL_CONFIGS = {
        ...
        "Qwen/Qwen2.5-Math-1.5B": GenerationConfig(
            max_tokens=3000,
            temperature=0.7,
            top_p=0.8,  # Optimal setting for math
            top_k=-1,
        ...
        "Qwen/Qwen2.5-Math-7B": GenerationConfig(
            max_tokens=3000,
            temperature=0.7,
            top_p=0.8,  # Optimal setting for math
            top_k=-1,
        ...
        "mistralai/Ministral-3-3B-Instruct-2512": GenerationConfig(
            max_tokens=131072,
            temperature=0.1,
            top_p=1.0,
            top_k=-1,
